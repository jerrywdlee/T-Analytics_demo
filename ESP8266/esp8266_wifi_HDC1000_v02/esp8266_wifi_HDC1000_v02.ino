...
void HTTP_GET_1(const char *url,const char *uuid,int remain_lvl,int battery_lvl,int opened)
{
    ...
    if((WiFi.status() == WL_CONNECTED)) {
        ...
        //温湿度センサーAdafruit_HDC1000
        //USE_SERIAL.println(hdc.readTemperature());
        //USE_SERIAL.println(hdc.readHumidity());
        //buffer for temp&hum floats to strings
        char temp[16];
        char hum[16];
        floatToString(temp, hdc.readTemperature() , 4);
        floatToString(hum, hdc.readHumidity() , 4);
        //const char *temp_s = floatToString(temp, hdc.readTemperature() , 3);
        //USE_SERIAL.println(temp_s);
        //USE_SERIAL.println(temp);
        //const char *hum_s = floatToString(hum, hdc.readHumidity() , 3);
...
void setup_client() {
  ...
  //int battery_lvl = 4;
  //電池電圧５回測定の平均
  int battery_lvl = get_battery_lvl(5);
  host.trim();
  uuid.trim();
...
//電池電圧関数
int get_battery_lvl(int times){
  //LEDピンを使ってMOSFETをONに
  digitalWrite(LED_PIN, HIGH);
  int temp_battery_lvl = 0;
  Serial.println(analogRead(A0));
  for(int i=0; i< times ; i++){
    temp_battery_lvl += analogRead(A0);
    //Serial.println(temp_battery_lvl);
    delay(100);
  }
  return temp_battery_lvl/times ;
}
